...
import { Heart, Gift, CreditCard, FileText, ChevronDown } from 'lucide-react';
...
export default function SupportPage() {
    ...
    const handleReveal = () => {
        setShowDetails(true);
        // Smooth scroll to details if needed, but the state change is usually enough
    };
        ...
        <div className="bg-slate-50 min-h-screen">
            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl -mt-16 relative z-20 pb-20">
                <FadeIn delay={0.2}>
                    ...
                    <div className="bg-white rounded-3xl p-8 sm:p-12 mb-12 shadow-xl border border-slate-100 flex flex-col md:flex-row items-center justify-between gap-8 transition-all duration-500">
                        <div className="max-w-2xl">
                            <h2 className="text-2xl font-bold text-gray-900 mb-3">Want to understand our long-term goals?</h2>
                            <p className="text-gray-600 text-lg">
                                Before committing your support, we invite you to read our detailed whitepaper. It outlines our strategic roadmap, impact metrics, and sustainability model for the year.
                            </p>
                            {!showDetails && (
                                <button
                                    onClick={() => setShowDetails(true)}
                                    className="text-blue-600 font-medium mt-4 hover:underline text-sm inline-flex items-center"
                                >
                                    Skip to donation details <ChevronDown className="w-4 h-4 ml-1" />
                                </button>
                            )}
                        </div>
                        <div className="flex-shrink-0">
                            <Link
                                ...
                                className="inline-flex items-center justify-center px-8 py-4 text-lg font-bold text-white bg-blue-600 rounded-full hover:bg-blue-700 transition-all hover:scale-105 shadow-lg group"
                            >
                                <FileText className="w-5 h-5 mr-2" /> Download Whitepaper
                            </Link>
                        </div>
                    </div>
